var template = ({name, github, description}) => `---
name: ${name}
title: TODO
active: false
affiliation:
...
function genPerson(person) {
  ...
  const doc = template(person)

  const filename = name.toLowerCase().replace(/\s+/g, '-')
  const filepath = path.join(__dirname, `${filename}.md`)

  fs.writeFile(filepath, doc, (err) => {
    if(err) console.log('Error saving', filepath)
    else console.log('Wrote', filepath)
  })
}

Object.keys(data).map((key) => data[key]).map((people) => people.map(genPerson))
